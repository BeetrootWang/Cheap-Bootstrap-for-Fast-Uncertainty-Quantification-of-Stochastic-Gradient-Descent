from main import main_experiments_parallel
import numpy as np
import time
        ...
        for d in [1000, 2000]:
            ...
            for eta in [0.02]:
                for cov_a_str in ['identity']:
                    ...
                    main_experiments_parallel(d, n, eta, alpha, x_star, x_0, B, var_epsilon, cov_a_str, num_trials)
            end = time.time()
            tmp_time = (end - start) / 10
            f = open(f"time_COfB{B}.txt", "a")
