...
for Cov in ["identity", "toeplitz", "equi"]:
    ...
    for B in [3,5,10]:
        ...
        for target_d in [5,20,200]:
            ...
            best_len = 1e5
            best_len_var = 0
            filename = f'./results_1109/Result_COB{B}_{target_d}_{Cov}.txt'
            # print(filename)
            f = open(filename, 'r')
            ...
            for line in f:
                if '-->' in line:
                    cnt = 1
                    if R_or_Ratio!='something wrong' and 1000*abs(cov-0.95)  < 1000*abs(best_cov - 0.95) :
                        best_cov = cov
