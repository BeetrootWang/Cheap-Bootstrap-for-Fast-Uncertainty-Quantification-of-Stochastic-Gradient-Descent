...
# compute bootstrap confidence interval
# center using \hat{x}
def bootstrap_CI_COfB_hat_center(x_0, n, R, a_n_history, b_n_history, eta, alpha):
    d = len(x_0)
    bootstrap_output_history = []
    rng_b = np.random.default_rng(1)  # random generator for bootstrap experiment
    bootstrap_samples_all = rng_b.integers(0, n, (R, n))  # bootstrap_samples[i] is the index of data for i-th iteration
    ...
    CfOB_Estimator = np.mean((bootstrap_output_history - np.repeat(np.reshape(x_0,(1,d)), R, axis=0))**2, axis=0)
    z = t.ppf(0.975,R)
    CI_radius = z * np.sqrt(CfOB_Estimator)
...
def run_SGD_LR_RS(seed, x_star, x_prev, n, eta, var_epsilon, mean_a, cov_a, alpha):
    rng = np.random.default_rng(seed)
    d = len(x_prev)
    x_history = []
    a_n_history = rng.multivariate_normal(mean=mean_a, cov=cov_a, size=(n))
...
def run_SGD_LR_COB(seed, x_star, x_prev, n, B, eta, var_epsilon, mean_a, cov_a, alpha):
    rng = np.random.default_rng(seed)
    d = len(x_prev)
    x_history = []
    x_B_prev = np.repeat(np.reshape(x_prev,(1,d)), B, axis=0)
...
def run_SGD_LR_HiGrad22(seed, x_star, x_prev, n, eta, var_epsilon, mean_a, cov_a, alpha):
...
def main_loop_COfB_hat_center(seed, x_star, x_0, n, R, eta, var_epsilon, mean_a, cov_a, alpha, num_trials):
    print(f'Seed: [{seed}/{num_trials}] ...')
    x_out, a_n_history, b_n_history = run_SGD_LR_O(seed, x_star, x_0, n, eta, var_epsilon, mean_a, cov_a, alpha)
    CI_radius = bootstrap_CI_COfB_hat_center(x_0, n, R, a_n_history, b_n_history, eta, alpha)
...
def main_loop_RS(seed, x_star, x_0, n, eta, var_epsilon, mean_a, cov_a, alpha, num_trials):
    print(f'Seed: [{seed}/{num_trials}] ...')
    x_out, CI_radius = run_SGD_LR_RS(seed, x_star, x_0, n, eta, var_epsilon, mean_a, cov_a, alpha)
...
def main_loop_COB(seed, x_star, x_0, n, B, eta, var_epsilon, mean_a, cov_a, alpha, num_trials):
    print(f'Seed: [{seed}/{num_trials}] ...')
    x_out, CI_radius = run_SGD_LR_COB(seed, x_star, x_0, n, B, eta, var_epsilon, mean_a, cov_a, alpha)
    mean_Len = np.mean(CI_radius * 2)
    std_Len = np.std(CI_radius * 2)
    cover = [1 if abs(x_out[ii] - x_star[ii]) <= CI_radius[ii] else 0 for ii in range(len(x_out))]
...
def main_loop_HiGrad22(seed, x_star, x_0, n, eta, var_epsilon, mean_a, cov_a, alpha, num_trials):
    print(f'Seed: [{seed}/{num_trials}] ...')
    CI_center, CI_radius = run_SGD_LR_HiGrad22(seed, x_star, x_0, n, eta, var_epsilon, mean_a, cov_a, alpha)
...
def main_experiments_parallel(d, n, eta, alpha, x_star, x_0, R, var_epsilon, cov_a_str, num_trials):
    ...
    # SGD origial loop
    # set random seed for original samples
    results = Parallel(n_jobs=1)(delayed(main_loop)(seed, x_star, x_0, n, R, eta, var_epsilon, mean_a , cov_a, alpha, num_trials) for seed in range(1, 1+num_trials))
    mean_len_history = []
    std_len_history = []
    len_history = []
    cov_history = []
...
def main_experiments_parallel_std(d, n, eta, alpha, x_star, x_0, R, var_epsilon, cov_a_str, num_trials):
    ...
    # SGD origial loop
    # set random seed for original samples
    results = Parallel(n_jobs=1)(delayed(main_loop_std)(seed, x_star, x_0, n, R, eta, var_epsilon, mean_a, cov_a, alpha, num_trials) for seed in range(1, 1+num_trials))
    mean_len_history = []
    std_len_history = []
    len_history = []
    cov_history = []
...
def main_experiments_parallel_COfB_hat_center(d, n, eta, alpha, x_star, x_0, R, var_epsilon, cov_a_str, num_trials):
    # mean and variance for generating a_i
    # identity covariance matrix case
    #
    # linear regression model:
    ...
    # SGD origial loop
    # set random seed for original samples
    results = Parallel(n_jobs=24)(delayed(main_loop_COfB_hat_center)(seed, x_star, x_0, n, R, eta, var_epsilon, mean_a , cov_a, alpha, num_trials) for seed in range(1, 1+num_trials))
    mean_len_history = []
    std_len_history = []
    len_history = []
    cov_history = []
    ...
    f = open(f'Result_COfBHat_{d}_{cov_a_str}.txt', 'a')
    f.write('----->\n')
    f.write(
        f'\t Cov Rate: {np.mean(cov_history)} \t ({np.std(cov_history)}) \tAvg Len: {np.mean(len_history)} \t ({np.std(len_history)/num_trials}) \n')
    f.write(f'\t d: {d} \t n: {n} \t R: {R} \t eta_0: {eta} \t alpha: {alpha} \t # Trials: {num_trials}\n')
...
def main_experiments_parallel_plug_in(d, n, eta, alpha, x_star, x_0, var_epsilon, cov_a_str, num_trials):
    ...
    # SGD origial loop
    # set random seed for original samples
    results = Parallel(n_jobs=1)(delayed(main_loop_plug_in)(seed, x_star, x_0, n, eta, var_epsilon, mean_a, cov_a, alpha, num_trials) for seed in range(1, 1+num_trials))
    mean_len_history = []
    std_len_history = []
    len_history = []
    cov_history = []
...
def main_experiments_parallel_RS(d, n, eta, alpha, x_star, x_0, var_epsilon, cov_a_str, num_trials):
    # mean and variance for generating a_i
    # identity covariance matrix case
    #
    # linear regression model:
    ...
    # SGD origial loop
    results = Parallel(n_jobs=1)(delayed(main_loop_RS)(seed, x_star, x_0, n, eta, var_epsilon, mean_a, cov_a, alpha, num_trials) for seed in range(1, 1+num_trials))
    mean_len_history = []
    std_len_history = []
    len_history = []
    cov_history = []
    ...
    f = open(f'Result_RS_{d}_{cov_a_str}.txt', 'a')
    f.write('----->\n')
    f.write(
        f'\t Cov Rate: {np.mean(cov_history)} \t ({np.std(cov_history)}) \tAvg Len: {np.mean(len_history)} \t ({np.std(len_history)/num_trials}) \n')
...
def main_experiments_parallel_BM(d, n, eta, alpha, x_star, x_0, M_ratio, var_epsilon, cov_a_str, num_trials):
    ...
    results = Parallel(n_jobs=1)(delayed(main_loop_BM)(seed, x_star, x_0, M, N, n, eta, var_epsilon, mean_a, cov_a, alpha, num_trials) for seed in range(1, 1+num_trials))
    mean_len_history = []
    std_len_history = []
    len_history = []
    cov_history = []
...
def main_experiments_parallel_wo(d, n, eta, alpha, x_star, x_0, R, var_epsilon, cov_a_str, num_trials):
    ...
    # SGD origial loop
    # set random seed for original samples
    results = Parallel(n_jobs=1)(delayed(main_loop_wo)(seed, x_star, x_0, n, R, eta, var_epsilon, mean_a, cov_a, alpha, num_trials) for seed in range(1, 1+num_trials))
    mean_len_history = []
    std_len_history = []
    len_history = []
    cov_history = []
...
def main_experiments_parallel_OB(d, n, eta, alpha, x_star, x_0, B, var_epsilon, cov_a_str, num_trials):
    ...
    # SGD origial loop
    # set random seed for original samples
    results = Parallel(n_jobs=1)(delayed(main_loop_OB)(seed, x_star, x_0, n, B, eta, var_epsilon, mean_a, cov_a, alpha, num_trials) for seed in range(1, 1+num_trials))
    mean_len_history = []
    std_len_history = []
    len_history = []
    cov_history = []
...
def main_experiments_parallel_COB(d, n, eta, alpha, x_star, x_0, B, var_epsilon, cov_a_str, num_trials):
    # mean and variance for generating a_i
    # identity covariance matrix case
    #
    # linear regression model:
    ...
    # SGD origial loop
    # set random seed for original samples
    results = Parallel(n_jobs=1)(delayed(main_loop_COB)(seed, x_star, x_0, n, B, eta, var_epsilon, mean_a, cov_a, alpha, num_trials) for seed in range(1, 1+num_trials))
    mean_len_history = []
    std_len_history = []
    len_history = []
    cov_history = []
    ...
    f = open(f'Result_COB{B}_{d}_{cov_a_str}.txt', 'a')
    f.write('----->\n')
    f.write(
        f'\t Cov Rate: {np.mean(cov_history)} \t ({np.std(cov_history)}) \tAvg Len: {np.mean(len_history)} \t ({np.std(len_history)/num_trials}) \n')
    f.write(f'\t d: {d} \t n: {n} \t B: {B} \t eta_0: {eta} \t alpha: {alpha} \t # Trials: {num_trials}\n')
...
def main_experiments_parallel_simple_rerun(d, n, eta, alpha, x_star, x_0, B, var_epsilon, cov_a_str, num_trials):
    ...
    # SGD origial loop
    # set random seed for original samples
    results = Parallel(n_jobs=1)(delayed(main_loop_simple_rerun)(seed, x_star, x_0, n, B, eta, var_epsilon, mean_a, cov_a, alpha, num_trials) for seed in range(1, 1+num_trials))
    mean_len_history = []
    std_len_history = []
    len_history = []
    cov_history = []
...
def main_experiments_parallel_HiGrad22(d, n, eta, alpha, x_star, x_0, var_epsilon, cov_a_str, num_trials):
    # mean and variance for generating a_i
    # identity covariance matrix case
    #
    # linear regression model:
    ...
    # SGD origial loop
    # set random seed for original samples
    results = Parallel(n_jobs=1)(delayed(main_loop_HiGrad22)(seed, x_star, x_0, n, eta, var_epsilon, mean_a , cov_a, alpha, num_trials) for seed in range(1, 1+num_trials))
    mean_len_history = []
    std_len_history = []
    len_history = []
    cov_history = []
    ...
    f = open(f'Result_HiGrad22_{d}_{cov_a_str}.txt', 'a')
    f.write('----->\n')
    f.write(
        f'\t Cov Rate: {np.mean(cov_history)} \t ({np.std(cov_history)}) \tAvg Len: {np.mean(len_history)} \t ({np.std(len_history)/num_trials}) \n')
    f.write(f'\t d: {d} \t n: {n} \t R: N.A. \t eta_0: {eta} \t alpha: {alpha} \t # Trials: {num_trials}\n')
